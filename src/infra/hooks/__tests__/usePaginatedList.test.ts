...
describe('usePaginatedList', () => {
  it('returns all pages together and stops fetching when there are no more pages', async () => {
    ...
    await waitFor(() =>
      expect(result.current.list).toStrictEqual(page1.concat(page2)),
    );

    result.current.fetchNextPage();
    expect(mockedGetList).toHaveBeenCalledWith(2);
